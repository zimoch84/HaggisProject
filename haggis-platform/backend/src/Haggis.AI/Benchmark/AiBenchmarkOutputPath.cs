...
namespace Haggis.AI.Benchmark
{
    public static class AiBenchmarkOutputPath
    {
        public static string WithRunSuffix(string path, AiBenchmarkOptions options, DateTime timestamp)
        {
            ...
            var strategy = SanitizeFileNamePart(options?.Strategy ?? "unknown");
            var opponent = SanitizeFileNamePart(options?.Opponent ?? "unknown");
            var suffix = $"{timestamp:yyyyMMdd_HHmmss}_{strategy}-vs-{opponent}";
