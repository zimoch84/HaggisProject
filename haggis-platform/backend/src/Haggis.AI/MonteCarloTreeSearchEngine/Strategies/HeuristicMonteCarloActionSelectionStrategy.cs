...
{
    public sealed class HeuristicMonteCarloActionSelectionStrategy : IMonteCarloActionSelectionStrategy
    {
        private readonly HeuristicActionRanker _ranker;
        private readonly int _topN;

        public HeuristicMonteCarloActionSelectionStrategy(HeuristicOptions heuristicOptions, int topN)
        {
        ...
        public IList<MonteCarloHaggisAction> Select(RoundState state, IList<MonteCarloHaggisAction> generatedActions)
        {
            ...
            if (ranked.Count <= _topN)
            {
                return ranked.Select(action => MonteCarloHaggisAction.FromHaggisAction(action.Action)).ToList();
            }

            var selected = ranked.Take(_topN)
                .Select(action => MonteCarloHaggisAction.FromHaggisAction(action.Action))
                .ToList();
            AppendFinalActions(generatedActions, selected);
            return selected;
        }
        ...
        private static void AppendFinalActions(IList<MonteCarloHaggisAction> source, ICollection<MonteCarloHaggisAction> selected)
        {
            foreach (var action in source.Where(action => action.IsFinal && !selected.Contains(action)))
            {
                selected.Add(action);
            }
        }
