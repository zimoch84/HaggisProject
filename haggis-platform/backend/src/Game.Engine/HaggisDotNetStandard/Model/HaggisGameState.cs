...
namespace Haggis.Model
{
    public class HaggisGameState : IState<IHaggisPlayer, HaggisAction>
    {
        ...
        private void MoveCardsToDiscardForPlayer(IHaggisPlayer player)
        {
            ...
            // Zbierz wszystkie karty z akcji bieżącego TrickPlay (tylko nie-pass)
            ...
            {
                // Dodaj kopię listy kart do discard docelowego gracza
                target.AddToDiscard(new List<Card>(cardsToMove));
            }

            // Wyczyść bieżące TrickPlay, aby nie dublować przeniesienia kart
