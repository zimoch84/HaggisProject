...
namespace Haggis.Extentions
{
    public static class StringExtensions
    {
        ...
        public static Trick ToTrick(this string trickString)
        {
            try
            {
                ...
                switch (type)
                {
                    ...
                    default:
                        throw new ArgumentException($"Nieobsługiwany TrickType: {type}");
                }
        ...
        public static List<Card> ToCards(this string cardString)
        {
            ...
            // Usunięcie nawiasów kwadratowych oraz podział ciągu na karty
            var cardParts = cardString.Trim('[', ']').Split('|');
            var cards = new List<Card>();

            foreach (var part in cardParts)
            {
                var card = part.ToCard(); // Użycie metody ToCard do konwersji
                cards.Add(card);
            }
