...
public class TextBuffer
{
    ...
    // Przesunięcie zawartości bufora w górę o zadany amont wierszy
    private void ScrollUp(int lines)
    {
        if (lines <= 0) return;

        int move = Math.Min(lines, _height);
        // Przesuń wiersze w górę
    ...
    // Zapisuje tekst zaczynając od aktualnej pozycji kursora.
    // Podobne zachowanie do Console.Write: obsługa '\n' i '\r'.
    public void Write(string text, ConsoleColor? color = null)
    {
        ...
        for (int i = 0; i < text.Length; i++)
        {
            ...
            if (_cursorX >= _width)
            {
                // zawijanie do następnego wiersza
                _cursorX = 0;
                _cursorY++;
                if (_cursorY >= _height)
                {
